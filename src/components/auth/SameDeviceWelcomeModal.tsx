...
import { ShieldCheck, User, ArrowRight, Lock, Key } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/60 dark:bg-black/70 backdrop-blur-md flex items-center justify-center p-3.5 sm:p-4 overflow-y-auto">
      <div className="bg-white dark:bg-stone-900 max-w-md w-full rounded-2xl sm:rounded-[2.5rem] border border-stone-200 dark:border-stone-800 shadow-2xl p-5 sm:p-8 overflow-hidden text-center my-auto max-h-[92vh] text-stone-900 dark:text-stone-100">
        ...
        <div className="p-4 bg-stone-50 rounded-2xl border border-stone-200/80 mb-6 flex items-center gap-3 text-left">
          <div className="w-10 h-10 bg-emerald-600 text-white font-bold rounded-xl flex items-center justify-center shrink-0">
        ...
        <button
          onClick={onContinue}
          className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-sm hover:shadow-md mb-4"
        >
          <span>Continue</span>
        ...
        <div className="pt-4 border-t border-stone-100 flex items-center justify-between text-xs text-stone-500">
          ...
          {onOpenRecoveryOrReset && (
            <button
              ...
              className="text-stone-400 hover:text-stone-700 underline font-medium transition-colors"
            >
              Vault Options
            </button>
          )}
